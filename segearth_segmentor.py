...
def _bilinear_resize(input_tensor, size, *, align_corners=False):
    if getattr(input_tensor, "is_npu", False):
        if os.environ.get("SEG310_EXPLICIT_OPERATORS", "1") == "0":
        ...
        from ascend_310b_operators.upsample_bilinear2d.source import (
            upsample_bilinear2d_310b,
        )

        return upsample_bilinear2d_310b(
            input_tensor, size=size, align_corners=align_corners
        )
    return F.interpolate(
        input_tensor,
        size=size,
...
class SegEarthSegmentation(BaseSegmentor):
    ...
    def forward_feature(self, img, logit_size=None):
        if type(img) == list:
            img = img[0]
        if self.clip_type == 'BLIP':
            img = _bilinear_resize(
                img, size=(self.slide_crop, self.slide_crop), align_corners=False
            )
        ...
        if self.feature_up:
            ...
            up_h, up_w = image_features.shape[2], image_features.shape[3]
            if up_h != image_w or up_w != image_h:
                image_features = _bilinear_resize(
                    image_features, size=(image_w, image_h), align_corners=False
                )
        ...
        if logit_size == None:
            logits = _bilinear_resize(logits, size=img.shape[-2:], align_corners=False)
        else:
            logits = _bilinear_resize(logits, size=logit_size, align_corners=False)
    ...
    def forward_slide(self, img, img_metas, stride=112, crop_size=224):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                del crop_seg_logit
                if os.environ.get("SEG310_EMPTY_CACHE_EACH_CROP", "1") == "1":
        ...
        logits = _bilinear_resize(preds, size=img_size, align_corners=False)
